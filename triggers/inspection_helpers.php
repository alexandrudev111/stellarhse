...
function ReplaceInspectionCorrectiveActionsParamters($ca){
    $corrective_action_block = '$Action_Start
        Report Number: $inspection_number
        Event type : $inspection_type_name
        Date (mm/dd/yyyy): $inspection_date
...
//    var_dump($ca);
        ...
        $corrective_action_replace = str_replace("$" . "inspection_number", $ca->report_number.'<br/>', $corrective_action_replace);
         $corrective_action_replace = str_replace("$" . "inspection_type_name", $ca->event_type_name.'<br/>', $corrective_action_replace);
        $corrective_action_replace = str_replace("$" . "inspection_date", $ca->report_date.'<br/>', $corrective_action_replace);
...
function InsertIntoInspectionEmailLog($db, $email_log_id, $corrective_action){
    ...
    $stmt->bindParam(':hist_inspection_id',$corrective_action->hist_report_id);
    $stmt->execute();
    $result = $stmt->rowCount();
}

function CheckInInspectionEmailLog($db, $email_log_id, $corrective_action){
    $query = "select * from stellarhse_inspection.`inspection_email_log` where email_log_id = :email_log_id and hist_inspection_id = :hist_inspection_id";
    $stmt = $db->prepare($query);
    $stmt->bindParam(':email_log_id',$email_log_id);
    $stmt->bindParam(':hist_inspection_id',$corrective_action->hist_report_id);
    $stmt->execute();
