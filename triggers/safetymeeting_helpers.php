...
function ReplaceSafetyMeetingCorrectiveActionsParamters($ca){
    $corrective_action_block = '$Action_Start
        Report Number: $safetymeeting_number
        Event type : $safetymeeting_type_name
        Date (mm/dd/yyyy): $safetymeeting_date
...
//    var_dump($ca);
        ...
        $corrective_action_replace = str_replace("$" . "safetymeeting_number", $ca->report_number.'<br/>', $corrective_action_replace);
         $corrective_action_replace = str_replace("$" . "safetymeeting_type_name", $ca->event_type_name.'<br/>', $corrective_action_replace);
        $corrective_action_replace = str_replace("$" . "safetymeeting_date", $ca->report_date.'<br/>', $corrective_action_replace);
...
function InsertIntoSafetyMeetingEmailLog($db, $email_log_id, $corrective_action){
    ...
    $stmt->bindParam(':hist_safetymeeting_id',$corrective_action->hist_report_id);
    $stmt->execute();
    $result = $stmt->rowCount();
}

function CheckInSafetyMeetingEmailLog($db, $email_log_id, $corrective_action){
    $query = "select * from stellarhse_safetymeeting.`safetymeeting_email_log` where email_log_id = :email_log_id and hist_safetymeeting_id = :hist_safetymeeting_id";
    $stmt = $db->prepare($query);
    $stmt->bindParam(':email_log_id',$email_log_id);
    $stmt->bindParam(':hist_safetymeeting_id',$corrective_action->hist_report_id);
    $stmt->execute();
