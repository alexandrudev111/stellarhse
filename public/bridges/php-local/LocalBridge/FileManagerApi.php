...
//        $this->basePath = $this->canonicalizePath($basePath ?: dirname(__DIR__) . '/../../files');
        $this->basePath = $this->canonicalizePath($basePath ?: '/data/report_documents/stellarhse_hazard/attachments' );
    ...
    public function postHandler($query, $request, $files)
    {
        ...
        if (!isset($request['action'])
            ...
            if ($uploaded['success'] === true && $uploaded['max'] =='') {
                $response = $this->simpleSuccessResponse();
            }
            else if ($uploaded['max'] === 'max_size') {
                $files_name ='';
                foreach ($uploaded['files_not_uploaded'] as $file_name){
                    $files_name .= $file_name ." \n";
                }
                $response = $this->simpleErrorResponse('Can\'t upload file , Max Size is 20 MB And file not Uploaded :' .$files_name);
            }
        ...
        if(isset($_SESSION['report_type'])&& isset($_SESSION['user_id'])&& isset($_SESSION['org_id']) && isset($_SESSION['report']) ){
            ...
            if(!isset($_SESSION['report_id'])){
                ...
                if($path_draft !=''){
                    $this->recurse_copy_draft($this->basePath, $path_draft);
                }
            }
        }
...
//        }
        ...
        foreach ($_FILES as $file) {
            ...
            if($file['size'] > 20000000) {
                $max =  'max_size';
                array_push($files_not_uploaded, $file['name']) ;
            }
        ...
        $data =[success=> true, max =>$max,files_not_uploaded =>$files_not_uploaded];
        return $data;
    }
    ...
    private function copyAction($oldPaths, $newPath ,$newName)
    {
        $newPath = $this->basePath . $this->canonicalizePath($newPath) . DIRECTORY_SEPARATOR;
        foreach ($oldPaths as $oldPath) {
            if (!file_exists($this->basePath . $oldPath)) {
    ...
    */
    ...
    private function recurse_copy_draft($src, $dst) {
        $dir = opendir($src);
        @mkdir($dst);
        while (false !== ( $file = readdir($dir))) {
            if (( $file != '.' ) && ( $file != '..' )) {
                ...
                if (is_dir($src . '/' . $file)) {
                    $this->recurse_copy_draft($src . '/' . $file, $dst . '/' . $file);
                } else {
