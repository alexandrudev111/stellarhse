(function () {
    var controller = function ($scope, $rootScope, $uibModal, coreService, viewDataTablesService, $filter, $q, $state, uiGridConstants, uiGridExporterConstants, coreReportService) {
        $scope.isOpenArr = [{
        ...
        $scope.temp_report_type='Hazard';
        $scope.templatesTypes_acc = [];
        var currentTab = '';
        var tempParamObj = '';
...
//            json1.push(paramsJson);
        ...
        function OpenUserTemplate(id) {
            ...
            // $('#TemplateId-ast').css('display', 'none');
            // $('#Subject-ast').css('display', 'none');
            var element = document.getElementById('div_EmailTemplateBody');
            if (element.style.display == "none") {
                $("#div_EmailTemplateBody").css('display', 'block');
            }
            ...
            viewDataTablesService.getReportTemplateBody(data)
                .then(function (res) {
                    var json = res.data[0][0];
                    $('#TemplateId').val(json.template_id);
                    $('#allorgstempName').val(json.template_name);
                    $scope.templateObject.name = json.template_name;
                    $("#TemplateTypeId").val(json.template_type_id);
                    $("#TemplateTypeId input[value='" + json.template_type_id + "']").attr('checked', true);
                    OrgId = json.org_id;
                    tinyMCE.activeEditor.setContent('');

                    tinyMCE.activeEditor.setContent(json.template_body);
//                    fillTemplateBody(res.data[1]);
                }, function (err) {
                    // OpenErrorDialog(xr.responseText, 'GetReportTemplateBody', data);
                    console.error(err);
                });
...
//                $scope.default_templates = queues[1].data;
        ...
        function GetTemplates() {
            ...
            var data = {
                ...
                org_id: $scope.user.org_id,
                language_id: $scope.user.language_id,
                report_type: $scope.temp_report_type
            };
            viewDataTablesService.getTemplatesTypes(data)
                .then(function(res){
                    var arr=[{org_id:'DefaultTemplate', org_name: "Default Templates"}];
                    $.each(res.data, function (i, item) {
                        arr.push(item);
                    });
                    console.table(arr);
                    $scope.templatesTypes_acc = arr;
                    ...
                    viewDataTablesService.getTemplates(data)
                        .then(function (res) {
                             console.log(res);
                            $scope.templatesTypes = res.data;
                            if ($scope.templatesTypes_acc && $scope.templatesTypes) {
                                var DefaultTemp = $('#DefaultTemplate');
                                var SharedTemp = $('#SharedTemplate');
                                $(DefaultTemp).empty();
                                $(SharedTemp).empty();

                                angular.forEach($scope.templatesTypes, function (template, index) {
    ...
    controller.$inject = ['$scope', '$rootScope', '$uibModal', 'coreService', 'viewDataTablesService', '$filter', '$q', '$state', 'uiGridConstants', 'uiGridExporterConstants', 'coreReportService']
    angular.module('manageDocumentModule')
        .controller('ManageDocumentController', controller);
}());