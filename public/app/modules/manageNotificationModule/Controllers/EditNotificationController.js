(function () {
    var controller = function ($scope, constantService, manageNotificationService, $state, $filter, coreService, $q, $uibModal, $confirm, coreReportService, $stateParams, $interval) {
        var firstLoad = true;
        ...
        var init = function () {        
            $scope.assignedNotification = $stateParams.selectedRow;
            ...
            var notification_data = {
                org_id: $scope.assignedNotification.org_id,
                notification_trigger_number: $scope.assignedNotification.notification_trigger_number,
                reportType: $scope.assignedNotification.report_type_code
            };
            console.log(notification_data);
            $q.all([
                manageNotificationService.getNotificationModulesTypes(data),
                manageNotificationService.getNotificationTypes(data),
                manageNotificationService.getNotificationData(notification_data),
                ...
                $scope.notificationData = queues[2].data;
                console.log($scope.notificationData);
                $scope.assignedNotification.choosedNotificationModule = $filter('filter')($scope.notificationModulesTypes, {code:$scope.assignedNotification.report_type_code})[0].id;
                if($scope.assignedNotification.employee_id !== ''){
                    $scope.assignedNotification.choosedNotifiedCategory = 'user';
                    $scope.assignedNotification.notifiedSelectedUser = {
                        employee_id: $scope.assignedNotification.employee_id,
                        full_name: $scope.assignedNotification.employee_name
                    };
                } else {
                    $scope.assignedNotification.choosedNotifiedCategory = 'group';
                    $scope.assignedNotification.notifiedSelectedGroup = {
                        id: $scope.assignedNotification.group_id,
                        name: $scope.assignedNotification.group_name
                    };
                }
                $scope.notifiedFields = queues[3].data;
                $scope.emailTypes = queues[4].data;
        ...
        $scope.getFilterFields = function(){
            ...
            manageNotificationService.getEmailTemplates(emailTemplateData)
                ...
                if(firstLoad){
                    angular.forEach($scope.notifiedFilterFields, function(filter){
                        console.log(filter)
                        var selectedFilter = $filter('filter')($scope.notificationData.row1, {field_id: filter.field_id});
                        console.log(selectedFilter)
                        if(selectedFilter.length !== 0){
                            filter.choosed = true;
                            filter.table_key_value  = [];
                            angular.forEach(selectedFilter, function(value){
                                filter.table_key_value.push(value.table_key_value);
                            });
                            $scope.getFilterValues(filter, null);
                        }
                    });
                }
        ...
        $scope.getFilterFieldsByType = function(){
            if(firstLoad){
                firstLoad = false;
                $scope.assignedNotification.notifiedStandardFilterFields = $filter('filter')($scope.notifiedFilterFields, {type_code:'standard'}); 
                $scope.assignedNotification.notifiedSpecialFilterFields = $filter('filter')($scope.notifiedFilterFields, {type_code:'special'});
            }
            ...
            if(angular.isDefined(filter.field_id) && filter.field_id !== null && filter.field_id !== '')
                ...
                (impact_id !== null && impact_id !== "" && filter.field_name === 'impact_sub_type_id'))
                    ...
                    .then(function(response){
                        ...
                        angular.forEach(filter.table_key_value, function(value){
                            var selectedValue = $filter('filter')(filter.filterValues, {id: value});
                            if(selectedValue.length !== 0){
                                var index = filter.filterValues.indexOf(selectedValue[0]);
                                filter.filterValues[index].choosed = true;
                            }
                        });
                    }, function(error){
                        coreService.resetAlert();
        ...
        $scope.updateNotification = function(){
            ...
            console.log($scope.assignedNotification);
            manageNotificationService.updateNotification($scope.assignedNotification)
            .then(function(response){
    ...
    controller.$inject = ['$scope', 'constantService', 'manageNotificationService', '$state', '$filter', 'coreService', '$q', '$uibModal', '$confirm', 'coreReportService', '$stateParams', '$interval'];
    angular.module('manageNotificationModule')
            .controller('EditNotificationController', controller);
}());
